...
class LabelSmoothedCrossEntropyCriterionAugmentConfig(
    ...
    mixedit_augmented_weight: float = field(
        default=1.0,
        metadata={"help": "loss weight for augmented samples"}
    )
    mixedit_regularization_weight: float = field(
        default=1.0,
        metadata={"help": "loss weight for KL regularization"}
    )
...
class AugmentedLabelSmoothedCrossEntropyCriterion(LabelSmoothedCrossEntropyCriterion):
    def __init__(
            ...
            mixedit_augmented_weight=1.0,
            mixedit_regularization_weight=1.0,
    ):
        super().__init__(
            task, sentence_avg, label_smoothing, ignore_prefix_size, report_accuracy
        )
        print(f"AugmentedLabelSmoothedCrossEntropyCriterion: "
              f"augmented_weight={mixedit_augmented_weight}, "
              f"regularization_weight={mixedit_regularization_weight},")
        self.augmented_weight = mixedit_augmented_weight
        self.regularization_weight = mixedit_regularization_weight
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if abs(self.augmented_weight) >= 1e-9:
            ...
            secondary_loss, secondary_nll_loss, secondary_sample_size = 0.0, 0.0, 0

        loss = primary_loss + self.augmented_weight * secondary_loss
        nll_loss = primary_nll_loss + self.augmented_weight * secondary_nll_loss
