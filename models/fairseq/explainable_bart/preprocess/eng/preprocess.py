...
class ExplanationBinarizer(Binarizer):
    """
    binarize
    """
    ...
    def binarize_line(
        ...
        logger.debug(line)
        splits = [int(x) for x in line.strip().split()]
        ids = torch.IntTensor(splits)
...
def _make_binary_explanation(
        input_prefix: str,
        output_prefix: str,
        num_workers: int,
        args: Namespace,
):
    binarizer = ExplanationBinarizer()
    input_file = "{}{}".format(input_prefix, ("." + "exp"))
    full_output_prefix = os.path.join(args.destdir, output_prefix)
    ...
    logger.info(f"[Explanation] {input_file} -> {output_prefix}: {final_summary}")
...
def _make_dataset(
    ...
    # Build Explanation
    if lang == "src":
        _make_binary_explanation(
            input_prefix, output_prefix + ".exp", num_workers, args
        )
