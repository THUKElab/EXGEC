...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if str.startswith(filename, "transformers:"):
            bart_model_file_from_transformers = filename.replace("transformers:", "")
            ...
            self.get_model().load_bart_state_dict_from_transformers(
                model, strict=True, args=self.cfg,
            )
            logger.info(f"loaded model from transformers: {bart_model_file_from_transformers}")
        ...
        if bexists:
            ...
            try:
                ...
                # User will fine tune the new roberta encoder via the ckpt saved above
